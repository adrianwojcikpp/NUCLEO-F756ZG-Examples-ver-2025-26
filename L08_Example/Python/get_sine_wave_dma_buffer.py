...
def main():
    ...
    c_array = Variable("sine_wave_dma_buffer", primitive="uint16_t", value=dac_reg)
    ...
    cw_inc.add_define("SINE_WAVE_DMA_BUFFER_SIZE", 100)
    cw_inc.add_variable_declaration(c_array, extern = True)

    with open("../Components/Src/sine_wave_dma_buffer.c", "w") as src:
        src.write("// PYTHON GENERATED FILE\n")
        src.write(str(cw_src))

    with open("../Components/Inc/sine_wave_dma_buffer.h", "w") as inc:
